...
module.exports.bootstrap = async function(cb) {
    try {
        ...
        for (let eventHandleFile of allEventHandleFiles) {
            let eventHandle = require(path.join(
                eventHandlersDir,
                eventHandleFile
            ));
            for (let listener of eventHandle) {
                EventEmitter.on(listener.eventName, data => {
                    sails.log.info(
                        `${eventHandleFile.replace(
                            '.js',
                            ''
                        )} - Executing event: ${listener.eventName}`
                    );
         ...
         *  Import users
         */
        // await ImportUserService.createImportUser();

        // await ImportUserService.deleteImportUser();
